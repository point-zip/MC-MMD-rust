...
fn parse_property<R: Read>(reader: &mut R) -> Result<FbxProperty> {
    ...
    match type_code {
        ...
        b'S' => {
            let len = reader
                .read_u32::<LittleEndian>()
                .map_err(|e| MmdError::FbxParse(format!("读取字符串长度失败: {}", e)))?
                as usize;
        ...
        b'R' => {
            let len = reader
                .read_u32::<LittleEndian>()
                .map_err(|e| MmdError::FbxParse(format!("读取二进制长度失败: {}", e)))?
                as usize;
            let mut buf = vec![0u8; len];
            reader
                .read_exact(&mut buf)
        ...
        _ => Err(MmdError::FbxParse(format!(
            "未知属性类型: 0x{:02X}",
            type_code
        ))),
    }
...
fn read_array_data<R: Read>(reader: &mut R, element_size: usize) -> Result<Vec<u8>> {
    let array_len = reader
        .read_u32::<LittleEndian>()
        .map_err(|e| MmdError::FbxParse(format!("读取数组长度失败: {}", e)))?
        as usize;
    let encoding = reader
        .read_u32::<LittleEndian>()
    ...
    let compressed_len = reader
        .read_u32::<LittleEndian>()
        .map_err(|e| MmdError::FbxParse(format!("读取压缩长度失败: {}", e)))?
        as usize;
...
fn parse_bool_array<R: Read>(reader: &mut R) -> Result<FbxProperty> {
    ...
    Ok(FbxProperty::BoolArray(
        data.into_iter().map(|b| b != 0).collect(),
    ))
}
