...
pub fn write_vmd<P: AsRef<Path>>(path: P, model_name: &str, motion: &Motion) -> Result<()> {
    let file = File::create(path.as_ref()).map_err(MmdError::Io)?;
...
pub fn write_vmd_to<W: Write>(w: &mut W, model_name: &str, motion: &Motion) -> Result<()> {
    // 文件头 (30 字节)
    w.write_all(VMD_HEADER).map_err(MmdError::Io)?;
    ...
    w.write_u32::<LittleEndian>(bone_count)
        .map_err(MmdError::Io)?;
    ...
    for (name, track) in &motion.bone_tracks {
        ...
        for kf in track.keyframes.values() {
            ...
            // 帧索引
            w.write_u32::<LittleEndian>(kf.frame_index)
                .map_err(MmdError::Io)?;
            // 平移：运行时读取做了 z = -tz，写入时要还原
            w.write_f32::<LittleEndian>(kf.translation.x)
                .map_err(MmdError::Io)?;
            w.write_f32::<LittleEndian>(kf.translation.y)
                .map_err(MmdError::Io)?;
            w.write_f32::<LittleEndian>(-kf.translation.z)
                .map_err(MmdError::Io)?;
            // 旋转：vmd_loader 读取时会执行 (rx, ry, -rz, -rw)
            // 为保证 round-trip 后内部四元数一致，这里写出时对 z/w 取反
            w.write_f32::<LittleEndian>(kf.orientation.x)
                .map_err(MmdError::Io)?;
            w.write_f32::<LittleEndian>(kf.orientation.y)
                .map_err(MmdError::Io)?;
            w.write_f32::<LittleEndian>(-kf.orientation.z)
                .map_err(MmdError::Io)?;
            w.write_f32::<LittleEndian>(-kf.orientation.w)
                .map_err(MmdError::Io)?;
            // 插值参数 (64 字节)
            ...
            w.write_all(&interp).map_err(MmdError::Io)?;
        }
    }
    ...
    w.write_u32::<LittleEndian>(morph_count)
        .map_err(MmdError::Io)?;
    ...
    for (name, track) in &motion.morph_tracks {
        ...
        for kf in track.keyframes.values() {
            ...
            w.write_u32::<LittleEndian>(kf.frame_index)
                .map_err(MmdError::Io)?;
            w.write_f32::<LittleEndian>(kf.weight)
                .map_err(MmdError::Io)?;
        }
    }

    // 相机关键帧 (0)
    w.write_u32::<LittleEndian>(0).map_err(MmdError::Io)?;
    // 光照 (0)
    w.write_u32::<LittleEndian>(0).map_err(MmdError::Io)?;
    // 阴影 (0)
    w.write_u32::<LittleEndian>(0).map_err(MmdError::Io)?;
    ...
    if motion.ik_tracks.is_empty() {
        ...
        w.write_u32::<LittleEndian>(frame_set.len() as u32)
            .map_err(MmdError::Io)?;

        let ik_names: Vec<&String> = motion.ik_tracks.keys().collect();
        for &frame in &frame_set {
            // 帧索引
            w.write_u32::<LittleEndian>(frame).map_err(MmdError::Io)?;
            // 显示标志 (1=显示)
            w.write_u8(1).map_err(MmdError::Io)?;
            // IK 信息数量
            w.write_u32::<LittleEndian>(ik_names.len() as u32)
                .map_err(MmdError::Io)?;
...
fn write_fixed_bytes<W: Write>(w: &mut W, data: &[u8], len: usize) -> Result<()> {
    if data.len() >= len {
        w.write_all(&data[..len]).map_err(MmdError::Io)?;
    } else {
        w.write_all(data).map_err(MmdError::Io)?;
        let pad = vec![0u8; len - data.len()];
        w.write_all(&pad).map_err(MmdError::Io)?;
    }
    Ok(())
}
