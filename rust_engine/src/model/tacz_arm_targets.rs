...
pub fn apply_tacz_arm_targets(
    ...
    if let Some(left) = targets.left {
        if let Some(diagnostic) = apply_side(
            bones,
            left,
            "左",
            LEFT_ARM_NAMES,
...
fn apply_side(
    ...
    let arm = find_first(bones, arm_names);
    let elbow = find_first(bones, elbow_names);
    let wrist = find_first(bones, wrist_names);
    ...
    let attachment = find_first(bones, attachment_names);
