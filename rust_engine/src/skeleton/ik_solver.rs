...
use super::bone_link::{BoneFlags, BoneLink, IkConfig, IkLink};
...
impl IkSolver {
    ...
    pub fn solve(&self, bones: &mut [BoneLink], children_cache: &[Vec<usize>]) {
        ...
        // 初始化 IK 链状态
        let mut chain_states: Vec<IkChainState> = self
            .config
            .links
            .iter()
    ...
    fn solve_iteration(
        ...
        for chain_idx in 0..self.config.links.len() {
            ...
            let chain_rot = bones[link_idx].ik_rotate
                * p.inverse()
                * bones[link_idx].animation_rotate
                * p
                * bones[link_idx].rest_rotation
                * delta_rot;
            ...
            bones[link_idx].ik_rotate = chain_rot
                * bones[link_idx].rest_rotation.inverse()
                * p.inverse()
    ...
    fn solve_plane(
        ...
        let angle = dot
            .acos()
            .clamp(-self.config.limit_angle, self.config.limit_angle);
